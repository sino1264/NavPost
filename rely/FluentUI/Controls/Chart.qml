...
Canvas {
    ...
    onPaint: {
        if(control.getContext('2d') !== null && d.memorizedContext !== control.getContext('2d') || d.memorizedData !== control.datas || d.memorizedOptions !== control.options) {
            var ctx = control.getContext('2d')
            d.jsChart =  Chart.build(ctx, {type: control.type,data: control.datas,options: control.options})
            d.memorizedData = control.datas
            d.memorizedContext = control.getContext('2d')
            d.memorizedOptions = control.options
