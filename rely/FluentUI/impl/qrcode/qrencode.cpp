...
unsigned char *FrameFiller_test(int version) {
    ...
    length = QRspec_getDataLength(version, QR_ECLEVEL_L) * 8 +
             QRspec_getECCLength(version, QR_ECLEVEL_L) * 8 + QRspec_getRemainder(version);
    for (i = 0; i < length; i++) {
...
STATIC_IN_RELEASE QRcode *QRcode_encodeMask(QRinput *input, int mask) {
    ...
    for (i = 0; i < raw->eccLength; i++) {
        ...
        for (j = 0; j < 8; j++) {
            ...
            if (p == NULL)
                goto EXIT;
            *p = 0x02 | static_cast<int>((bit & code) != 0);
            bit = bit >> 1;
        }
    }
    QRraw_free(raw);
...
static QRcode *QRcode_encodeDataReal(const unsigned char *data, int length, int version,
...
QRcode *QRcode_encodeString8bitMQR(const char *string, int version, QRecLevel level) {
    ...
    for (i = version; i <= MQRSPEC_VERSION_MAX; i++) {
        QRcode *code =
            QRcode_encodeDataReal((unsigned char *) string, (int) strlen(string), i, level, 1);
        if (code != NULL)
            return code;
    }
...
static QRcode_List *QRcode_encodeDataStructuredReal(int size, const unsigned char *data,
                                                    int version, QRecLevel level, int eightbit,
                                                    QRencodeMode hint, int casesensitive) {
    QRinput *input;
    QRcode_List *codes;
    int ret;
...
QRcode_List *QRcode_encodeDataStructured(int size, const unsigned char *data, int version,
                                         QRecLevel level) {
...
QRcode_List *QRcode_encodeStringStructured(const char *string, int version, QRecLevel level,
                                           QRencodeMode hint, int casesensitive) {
    if (string == NULL) {
        errno = EINVAL;
        return NULL;
